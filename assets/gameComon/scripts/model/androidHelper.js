...
module.exports = {
     ...
     * 获取平台
     */
    getPhonePlatFrom : function (){
        let platFrom =  jsb.reflection.callStaticMethod("org/cocos2dx/AndroidHelper","getPhonePlatFrom","()Ljava/lang/String;");
        return platFrom;
      },
    ...
    */
     ...
     */
     wechatShare : function(payment,cb){  
     ...
     * 播放网络音乐
     */ 
    playNetSound : function(info,cb){//info:url 地址
        cc.eventManager.removeCustomListeners('playNetSoundOver');
        cc.eventManager.addCustomListener('playNetSoundOver',function(event){//监听播放完毕返回
            cb(event.getUserData().a);
        });
     ...
     * 录音结束
     */ 
    playRecorderEnd : function(){//传入七牛网盘生成地址需要的url前缀，name文件名，token。
    ...
    uploadRecorder : function(){
        ...
        app.playerMgr.getRecordToken(function (data) {
            ...
            jsb.reflection.callStaticMethod("org/cocos2dx/SoundInfo","uploadRecorder",
            "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V",data.url,data.name,data.token);
        });
    },
     ...
     * 获取地理位置
     */ 
    getLocationInfo : function(cb){//经纬度以###分开
        if (app.creatorPlatformType ==1 ||app.creatorPlatformType ==2) {
            ...
            cc.eventManager.addCustomListener('baiDuLocationResult', function (event) {
                let pos = event.getUserData().a.split('###'); // 前面精度，后面纬度
                cb({ x: pos[0], y: pos[1] });
            });
            jsb.reflection.callStaticMethod("org/cocos2dx/Map", "getLocationInfo",
                "(Ljava/lang/String;)V", "cocos");
        } else {
            let pos = '108.883452###34.232702'.split('###'); // 前面精度，后面纬度
            cb({ x: pos[0], y: pos[1] });
        }
    },
    /**
    ...
    openSetting : function(){
        jsb.reflection.callStaticMethod("org/cocos2dx/AndroidHelper","openSetting",
            "()V");
    },
     /**
     * 获取手机电量
     */ 
    getPhoneElectric : function(cb){
     ...
     * 检测大版本更新 
     */
    checkApkUpdate : function(hotVersion,childVersion,cb){
  ...
  /**
     * 四种登录模式,第三方登陆
     */
    thirdLogin : function(info,cb){//info只用在yyb平台
   ...
   /**
    ...
    /**调用anroid层下载 */
    uploadApk : function(url,name,cb){
     ...
     */
    openInUrl : function(url,type,name,isDownload,playerId,adId){
